...
if useSvg:
  surface = cairo.SVGSurface("llod-cloud.svg",canvas,canvas)
else:
...
# .67 .84 .90
cairo_draw(g,pos,ctx,vertex_text=label, vertex_size=size, vertex_pen_width=8.0, vertex_color=[0.2,0.2,0.9,1.0], vertex_font_size=vertexBaseSize/2,vertex_fill_color=[1.0,1.0,1.0,1.0], vertex_text_color="black",vertex_text_position=-2, edge_pen_width=edgesize, edge_marker_size=40,fit_view=True,vertex_font_family="sans",vertex_font_weight=cairo.FONT_WEIGHT_BOLD)

ctx.move_to(canvas*0.02,canvas*0.95)
ctx.set_font_size(vertexBaseSize)
